(() => {
  ...
  function render() {
    ...
    root.innerHTML = `
      <header class="indicator-hero">
        <div>
          <div class="report-badges">
            ...
            <span class="badge">TradingView charts</span>
            <span class="badge">FedWatch snapshot</span>
          </div>
          <h2>기타 지표</h2>
          <p class="report-thesis">시장 전체 온도를 먼저 확인하고 ETF·주식 분석으로 들어가기 위한 보조판입니다. 환율, 원자재, 코인, 고중요 경제 이벤트, FOMC 확률을 한 화면에서 봅니다.</p>
        </div>
        <aside class="quote-panel indicator-status-panel">
          <small>스냅샷 기준</small>
          <strong>${escape(data.asOf)}</strong>
          <span class="quote-change flat">정적 기록 + 외부 차트</span>
      ...
      <div class="report-body indicator-body">
        ${renderSummary()}
        ${sectionHeader("01", "고중요 경제 이벤트", "Investing.com 경제 캘린더 중 시장 영향도가 큰 이벤트를 따로 모아 봅니다.", renderCalendar())}
        ${sectionHeader("02", "환율·크립토·원자재 차트", "달러/원, 엔/원, 유로/원, BTC, ETH, 금·은·구리·WTI·천연가스를 TradingView 규칙으로 표시합니다.", renderCharts())}
        ${sectionHeader("03", "FedWatch 확률", "다음 3개 FOMC의 목표금리 확률을 스냅샷으로 저장합니다.", renderFedWatch())}
        ${sectionHeader("04", "데이터 사용 원칙", "실시간성, 지연 시세, 정적 기록의 경계를 명확히 둡니다.", renderPolicy())}
      </div>`;
  ...
  function renderSummary() {
    ...
    return `<div class="indicator-summary">
      <div><small>경제 캘린더</small><strong>${data.calendar.events.length}개</strong><p>중요도 ★★★ 이벤트 스냅샷</p></div>
      <div><small>대표 차트</small><strong>${data.marketCharts.length}개</strong><p>환율·크립토·원자재</p></div>
  ...
  function renderCalendar() {
    ...
    return `<div class="calendar-shell">
      <div class="calendar-note">
        <span>기준 ${escape(calendar.asOf)}</span>
      ...
      <div class="event-table-wrap">
        <table class="event-table">
          <thead><tr><th>국가</th><th>이벤트</th><th>중요도</th><th>Actual</th><th>Forecast</th><th>Previous</th><th>메모</th></tr></thead>
          <tbody>
            ${calendar.events.map((event) => `<tr>
              <td><span class="country-pill">${escape(event.country)}</span></td>
  ...
  function renderCharts() {
    ...
    return `<div class="security-chart-shell indicator-chart-shell">
      <div class="security-chart-copy">
        <strong>TradingView 우선</strong>
        <p>표시 가능한 심볼은 페이지 안에 바로 띄우고, 외부 데이터 정책 때문에 막히면 TradingView 원문 링크로 확인합니다.</p>
      </div>
      <div class="security-chart-layout indicator-chart-layout">
        <div class="security-list indicator-list" role="group" aria-label="기타 지표 차트 선택">
          ${data.marketCharts.map((item) => `<button class="security-item indicator-item ${item.id === first.id ? "active" : ""}" data-indicator-id="${escapeAttr(item.id)}" aria-pressed="${item.id === first.id ? "true" : "false"}">
            ...
            <span class="security-item-side"><em>${escape(item.ticker)}</em><small class="chart-mode tv">${escape(item.category)}</small></span>
          </button>`).join("")}
        </div>
        <div class="security-chart-area">
          <div class="security-chart-meta indicator-chart-meta">
            <span>
              ...
              <small data-indicator-symbol>${escape(first.symbol)}</small>
              <small class="chart-provider">TradingView 차트</small>
            </span>
      ...
      <p class="data-note">실시간성은 TradingView와 각 거래소·데이터 제공자 정책을 따릅니다. 원자재 선물은 지연 시세일 수 있습니다.</p>
    </div>`;
  }
  ...
  function activateChart() {
    ...
    container.innerHTML = `<div class="external-chart-card">
      ...
      <p>TradingView 차트 로더를 찾지 못했습니다. 아래 링크에서 원문 차트를 확인하세요.</p>
